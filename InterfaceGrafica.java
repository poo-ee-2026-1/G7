...
import model.*;
import persistencia.ExportadorExcelCSV;
...
public class InterfaceGrafica extends JFrame {
    ...
    private void initComponents() {
        ...
        JButton btnExport  = createStyledButton("Exportar Excel");
        btnExport.addActionListener(e -> exportarParaExcel());

        navPanel.add(btnList);
        navPanel.add(btnSummary);
        navPanel.add(btnDelete);
        navPanel.add(btnExport);
    ...
    private void exportarParaExcel() {
        ...
        chooser.setDialogTitle("Salvar planilha CSV compatível com Excel");
        chooser.setSelectedFile(new File("transacoes_" + gerenciador.getUsuario().getEmail().replaceAll("[^a-zA-Z0-9]", "_") + ".csv"));
        ...
        try {
            new ExportadorExcelCSV().exportar(arquivo.toPath(), gerenciador.getUsuario(), gerenciador.listarTodas());
            JOptionPane.showMessageDialog(this,
            ...
            JOptionPane.showMessageDialog(this,
                    "Erro ao exportar para Excel/CSV: " + ex.getMessage(),
                    "Erro de exportação",
                    JOptionPane.ERROR_MESSAGE);
        }
    }
