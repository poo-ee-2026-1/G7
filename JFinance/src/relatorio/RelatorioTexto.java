...
public class RelatorioTexto {
    ...
    public void imprimirResumoGeral() {
        ...
        System.out.printf("  Total de transações : %d%n", gerenciador.getTotalTransacoes());
        System.out.printf("  Total de receitas   : R$ %,.2f%n", gerenciador.calcularTotalReceitas());
        System.out.printf("  Total de despesas   : R$ %,.2f%n", Math.abs(gerenciador.calcularTotalDespesas()));
        System.out.printf("  SALDO FINAL         : R$ %,.2f%n", gerenciador.calcularSaldo());
        System.out.println(LINHA2 + "\n");
    }

    public void imprimirRelatorioMensal(int ano, Month mes) {
        String nomeMes = mes.getDisplayName(TextStyle.FULL, Locale.forLanguageTag("pt-BR"));
        List<Transacao> lista = gerenciador.filtrarPorMes(ano, mes);

        System.out.println("\n" + LINHA2);
        System.out.printf("  RELATÓRIO MENSAL — %s/%d%n", nomeMes.toUpperCase(), ano);
        System.out.println(LINHA2);
        ...
        System.out.println(LINHA);
        System.out.printf("  SALDO DO MÊS: R$ %,.2f%n", gerenciador.calcularSaldoMensal(ano, mes));
        System.out.println(LINHA2 + "\n");
    }
